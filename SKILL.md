...
Triggered by `/great-design` with a one-line brief, by a description of the surface, or by this skill's description on its own. The brief needs the surface and audience, the constraints that matter (assets, fonts, design system, which data may appear on the page), and whether the user will be around to pick between rendered directions; when any of that is missing, ask for it first, then start Stage 1. With a user: three rendered options, then a refine pass. Without one: three written directions, the furthest from the default gets built. Report the rejected default, the brief, the critic model used, the last critic score with its open findings, and desktop plus phone screenshots. Scores of 6 to 7 are normal for constrained work; the loop stops by budget, not by chasing 9. A surface takes 20 to 85 minutes, the spread being the number of critic rounds, so this is for pages people judge by eye, not for a token tweak. Never invent data or names the source lacks; internal metrics reach the page only when the brief says so.
...
1. **Stop editing, then screenshot.** `node ${CLAUDE_SKILL_DIR:-$HOME/.claude/skills/great-design}/shoot.cjs <url|file> <out-prefix>`, with the prefix outside the project tree (the session scratchpad): desktop and phone PNGs, fonts reported, scroll reveals fired, reduced motion emulated. gstack `$B screenshot` works for a state already open in the browse daemon, at that tab's current width, with no reveal pass, font check or reduced-motion; do not compare its widths with the helper's. Screenshot any state that matters (hover, dirty, empty, error). Look at the PNGs yourself first: fix what you can see, re-shoot, then dispatch. The critic is for what you cannot see. A critic run while edits continue reviews a page that no longer exists.
2. **Dispatch the critic**: a fresh subagent on the model the harness names as its most capable (never `sonnet` or `haiku`), set explicitly and named in the report, given the PNGs and the brief (surface, audience, purpose, constraints, design system, the world). Behaviour a screenshot hides ("Save fills only when a value changes") may be stated as a fact. Not the source, not the previous round's findings or score, not what you fixed, not the target score.
...
| Critic | Fresh context, PNGs and brief, the most capable model named explicitly, facts not fixes | Source, prior findings, fixed-list, target score |
