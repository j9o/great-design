...
2. **Dispatch the critic**: a fresh subagent on the flagship tier of the Agent tool's `model` enum (never `sonnet` or `haiku`; when two flagship names are listed, the newer generation), set explicitly and named in the run log, given the PNGs and the brief (surface, audience, purpose, constraints, design system, the world). Behaviour a screenshot hides ("Save fills only when a value changes") may be stated as a fact. Not the source, not the previous round's findings or score, not what you fixed, not the target score.
